/**
 * AD5933.cpp - Library code for AD5933
 *
 * https://github.com/WuMRC/drive
 * https://github.com/mjmeli/arduino-ad5933
 *
 * @author Michael Meli
 * @modified by Christin Thürwächter, March 2019
 */
...
/**
 * Set the settling time cycles register, NUM_SCYCLES_1 and NUM_SCYCLES_2.
 *
 * @param mode The number of settling cycles to set
 * @return Success or failure
 */
bool AD5933::setSettlingCycles(int factor, int number) {
   ...
   // if (number > 511) {
    ...
    byte val1;
    byte val2;
    if (!getByte(NUM_SCYCLES_1, &val1))
        return false;
    if (!getByte(NUM_SCYCLES_2, &val2))
        return false;
    ...
    // Determine the selected multiplication factor and set it in bit D10 and D9
    switch (factor) {
        case 1:
            val1 |= NUM_SET_CYCLES_DEFAULT;
		break;
        case 2:
            val1 |= NUM_SET_CYCLES_2;
		break;
        case 4:
            val1 |= NUM_SET_CYCLES_4;
		break;

        default:
            return false;
    }
	Serial1.println(val1, BIN);
    // Take bit D9 and add to selected factor value
    byte upperByte = (number >> 8) & 0x01;
    val1 |= upperByte;
	Serial1.print("upper byte: ");
	Serial1.println(val1, BIN);
    // Take lower 8 bit of number
    val2 |= number & 0xFF;
	Serial1.print("lower byte: ");
	Serial1.println(val2, BIN);
    // Write back to the register
     sendByte(NUM_SCYCLES_1, val1);
     sendByte(NUM_SCYCLES_2, val2);
}
 ...
 * @return Success or failure
 */
bool AD5933::setStartFrequency(unsigned long start, bool internal) {
	...
	long freqHex;
	if (internal){
	freqHex = (start / (internalSpeed / 4.0))*pow(2, 27);
	if (freqHex > 0xFFFFFF) {
        	return false;   // overflow
    	 }
        }
    	else{
	freqHex = (start / (externalSpeed / 4.0))*pow(2, 27);
	if (freqHex > 0xFFFFFF) {
        	return false;   // overflow
   	 }
       	}
    ...
    //    return false;   // overflow
    //}
    	Serial1.print("Startfreq freqHex: ");
	Serial1.println(freqHex);
 ...
 * @return Success or failure
 */
bool AD5933::setIncrementFrequency(unsigned long increment, bool internal) {
	...
	long freqHex;
	if (internal){
	freqHex = (increment / (internalSpeed / 4.0))*pow(2, 27);
	...
	freqHex = (increment / (externalSpeed / 4.0))*pow(2, 27);
	if (freqHex > 0xFFFFFF) {
        	return false;   // overflow
    	 }
       	}
	...
	//long freqHex = (increment / (clockSpeed / 4.0))*pow(2, 27);
    	Serial1.print("Increment freqHex: ");
	Serial1.println(freqHex);
...
/**
 * Set the Excitation range level.
 *
 * @param level The Excitation range level to select. 
 * @return Success or failure
 */
bool AD5933::setExcitationRange(byte level) {
    // Get the current value of the control register
    byte val;
    if (!getByte(CTRL_REG1, &val))
        return false;

    // Clear out bit D9 and D10, which are the excitation range set bits
    val &= 0xF9;
	
    // Determine what excitation range was selected
    if (level == 1) {
        // Set excitation range to 1.98V p-p in CTRL_REG1
        val |= EXCITATION_LEVEL_1;
        return sendByte(CTRL_REG1, val);
    } else if (level == 2) {
        // Set excitation range to 0.97V p-p in CTRL_REG1
        val |= EXCITATION_LEVEL_2;
        return sendByte(CTRL_REG1, val);
    } else if (level == 3) {
        // Set excitation range to 0.38V p-p in CTRL_REG1
        val |= EXCITATION_LEVEL_3;
        return sendByte(CTRL_REG1, val);
    } else if (level == 4) {
        // Set excitation range to 0.2V p-p in CTRL_REG1
        val |= EXCITATION_LEVEL_4;
        return sendByte(CTRL_REG1, val);
    } else {
        return false;
    }
...
/**
 * Read the value of the number of settling time cycles register.
 *
 * @return The value of the number of settling time cycles register. Returns 0xFFFF if can't read it.
 */
int AD5933::readSCyclesRegister() {
    return ((readRegister(NUM_SCYCLES_1) << 8) | readRegister(NUM_SCYCLES_2));
}
...
bool AD5933::frequencySweep(int real[], int imag[], int n) {
	...
	Serial1.println("fail readStatus");
	return false;
        }
	...
	Serial1.println("fail getComplex");
	 return false;
        }
...
bool AD5933::calibrate(double gain[], int phase[], int ref, int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (imag[i]>=0 && real[i]>=0)
          phase[i] = atan(abs(imag[i])/abs(real[i]))*180/pi;
        
        if (imag[i]>=0 && real[i]<0)
          phase[i] = 180-(atan(abs(imag[i])/abs(real[i]))*180/pi);
        
        if (imag[i]<0 && real[i]<0)
          phase[i] = 180+(atan(abs(imag[i])/abs(real[i]))*180/pi);
        
        if (imag[i]<0 && real[i]>=0)
          phase[i] = 360-(atan(abs(imag[i])/abs(real[i]))*180/pi);
